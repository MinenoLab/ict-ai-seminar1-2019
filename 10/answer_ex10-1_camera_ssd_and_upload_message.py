...
# [10-1-1].自作のライブラリを読み込む
from ex10_upload_message import upload_message_to_slack
...
class detection:
    ...
    def __init__(self):
        ...
        # [10-1-2].Slackへ1度しかアップロードを行わないようにするためのアップロードフラグ
        self.is_upload = True
    ...
    def main(self):
        while self.cap.isOpened():
            ...
            # [10-1-3].rキーでSlackへのアップロードフラグをリセット
            if key == ord('r'):
                self.is_upload = True
            ...
            if self.flag_detection == True:
                ...
                for i in range(top_conf.shape[0]):
                    ...
                    if self.is_upload:  # [10-1-4].この行は絶対に書き換えない
                        # [10-1-5].検出されたラベルに応じて，Slackにアップロードするライブラリを呼び出す
                        if label_name in ["Cat", "Tvmonitor"]:  # [10-1-6].この行を自分で書き換える
                            upload_message_to_slack(label_name) # [10-1-7].この関数に渡す引数を自分で書き換える
